...
""" I learned how to use monkeypatch from this Medium post:
https://pavolkutaj.medium.com
/simulating-single-and-multiple-inputs-using-pytest-and-monkeypatch-6968274f7eb9
"""
def test_addMissedQuestion(monkeypatch):
    ex = Exam("Final!!!", pointsPossible=100, pointsEarned=88)
    assert not ex.questions
    ...
    assert ex.questions
    assert ex.questions["some question"] == "an answer"
    ...
    ex.addMissedQuestion()
    assert ex.questions["some question"] == "an answer"
    assert ex.questions["another one"] == "another answer"
